    ...
    fn poll(self: std::pin::Pin<&mut Self>, cx: &mut std::task::Context<'_>) -> std::task::Poll<Self::Output> {
        ...
        match &mut this.kind {
            Kind::Preflight { headers } => {
                let mut resp = Response::builder().body(<ResBody as Default>::default()).unwrap_or_default();
            ...
            Kind::Cors { header, fut } => {
                ...
                Poll::Ready(res.map(|mut x| { x.headers_mut().extend(header.drain()); x}))
            },
            Kind::Inmediate { res } => {
                Poll::Ready(Ok(res.take().unwrap_or_default()))
            }
        }
    }
