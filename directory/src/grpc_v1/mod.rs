...
pub use proto::{AllowedBucketReq, BucketReq, info_client::InfoClient, Permissions};
use uuid::Uuid;
...
impl ConnectionAuthMS {
    ...
    pub async fn allowed(&self, id: Uuid, name: String, permission: Permissions) -> bool {
        self.inner.clone()
            .bucket_is_allowed(AllowedBucketReq{id: id.as_bytes().to_vec(), name, permissions: permission as i32})
            .await
    ...
    pub async fn buckets_user(&self, id: Uuid) -> Option<UserReply> {
        self.inner.clone()
            .user_by_id(UserByIdReq{ id: id.as_bytes().to_vec() })
