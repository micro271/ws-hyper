...
pub struct RenameControl {
    r#await: u64,
    sender_watcher: <EventWatcher as Actor>::ActorRef,
    tasks: HashMap<PathBuf, JoinHandle<()>>,
}

impl RenameControl {
    pub fn new(sender_watcher: <EventWatcher as Actor>::ActorRef, r#await: u64) -> Self {
        Self {
            sender_watcher,
            r#await,
            tasks: HashMap::new(),
    ...
    async fn handle(&mut self, message: Self::Message, _ctx: &mut Self::Context) -> Self::Reply {
        ...
        match message {
            Rename::From(RenameFrom(from)) => {
                ...
                let self_ref = _ctx.actor_ref().clone();
