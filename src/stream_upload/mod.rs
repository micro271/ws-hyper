...
where
    T: AsyncWrite,
{
    pub async fn new(multipart: Multipart<'a>, fd: T, allowed: Vec<Mime>) -> Self {
        Self {
            multipart,
            ...
            match &mut this.state {
                ...
                State::ReadingField(field) => match field.poll_next_unpin(cx) {
                    Poll::Ready(Some(Ok(bytes))) => {
                        ...
                        if let Poll::Ready(Err(e)) = writer.poll_write(cx, &bytes) {
                    ...
                    Poll::Ready(None) => this.state = State::WaitingField,
                    Poll::Pending => break Poll::Pending,
                },
