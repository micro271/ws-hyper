...
async fn main() -> Result<(), Box<dyn std::error::Error>>{
    ...
    let listen = TcpListener::bind(socket.clone()).await?;
    ...
    loop {
        ...
        tokio::task::spawn(async move {
            if let Err(e) = hyper::server::conn::http1::Builder::new()
                .serve_connection(io, service_with_state(repo, |req, repo| handlers::middlewares(req, peer, repo))).await {
                println!("{:?}", e);
            }
        });
    }
...
pub fn service_with_state<F, R, S>(state: Arc<Repository>, f: F) -> ServiceWithState<F,R> 
    where 
        ...
        S: Future,
{
    ServiceWithState { f, state, _req: PhantomData }
}
...
impl<F: Clone, R> std::clone::Clone for ServiceWithState<F, R> {
    fn clone(&self) -> Self {
        ServiceWithState { f: self.f.clone(), state: self.state.clone(), _req: PhantomData }
    }
}
...
impl<T: tokio::io::AsyncRead> hyper::rt::Read for Io<T> {
    fn poll_read(
        ...
        let tmp = unsafe {
            ...
            match tokio::io::AsyncRead::poll_read(Pin::new_unchecked(&mut self.get_unchecked_mut().inner), cx, &mut tbuf){
...
impl<T: tokio::io::AsyncWrite> hyper::rt::Write for Io<T> {
    fn poll_write(
        ...
        tokio::io::AsyncWrite::poll_write(unsafe {
    ...
    fn poll_flush(self: Pin<&mut Self>, cx: &mut std::task::Context<'_>) -> std::task::Poll<Result<(), std::io::Error>> {
        tokio::io::AsyncWrite::poll_flush(unsafe {
            Pin::new_unchecked(&mut self.get_unchecked_mut().inner)
        }, cx)
    }
    ...
    fn poll_shutdown(
        ...
        tokio::io::AsyncWrite::poll_shutdown(unsafe {
            Pin::new_unchecked(&mut self.get_unchecked_mut().inner)
        }, cx)
    }
}